...
\chaptermark{Preschool convention formation}

```{r preschool-setup, include=FALSE}
...
expt_1 <- "kids/data/expt_1"
expt_2 <- "kids/data/expt_2"
mod_loc <- "kids/code/models"
mod_results <- "kids/code/models/summary"
mod_form <- "kids/code/models/formulae"
time_palette <- c("practice" = "grey", "block 1" = "#E41A1C", "block 2" = "#377EB8", "block 3" = "#4DAF4A", "block 4" = "purple") 


source(here("kids/code/paper_helper.R"))
```
...
Iterated reference games provide an important paradigm for studying referential communication. In these games, one player repeatedly describes a set of abstract shapes to a partner so they can identify the target images [@clark1986; @krauss1964; @hawkins2020; @boyce2024]. Over repetition, features of the initial descriptions are conventionalized as each pair comes to agree on a shared understanding of how to label each image. Success at this task requires mastery of a number of linguistic and communicative skills, including producing adequate initial descriptions, monitoring for comprehension, asking for clarification, and appropriately using the shared conversation history to inform later referring expressions. 
...
```{r interface, fig.cap = "Experimental setup and procedure. Panel A shows the experimental setup. Panel B shows the 4 possible targets; names for targets are for cross-reference with later figures only. Panel C shows the procedure for Experiment 1; within critical blocks targets were ordered randomly. Panel D shows the procedure for Experiment 2. \\label{game-kid}", cache=FALSE}

knitr::include_graphics(here("kids/diagram.pdf"))
...
```{r fig-accuracy, fig.cap = "Children's accuracy at selecting the correct target over time. Error bars are bootstrapped 95% CIs with a linear trend line overlaid. \\label{acc}" }
clean_2 |>
...
```{r description, fig.cap = "Length of description produced by the teller each trial. Grey dots are individual data points, colored dots are per trial means with bootstrapped 95% CIs. \\label{length}" }
clean_2 |>
  filter(type != "practice") |>
  mutate(trial = trial - 3, expt = "Expt 2") |>
  bind_rows(clean_1 |> filter(type != "practice") |> mutate(trial = trial - 1, expt = "Expt 1")) |>
...
```{r sims, fig.cap = "Semantic similarity between pairs of descriptions from different sources. Dots are means and lines are bootstrapped 95% CIs.  \\label{sim}" }
sims_2 |>
...
\begin{table}
	...
	\begin{tabular}[t]{p{16em}p{4em}}
		\hline
		%hold
		$\bullet$ person & 		\multirow{6}{*}{\includegraphics[width=.5in]{../kids/I1.jpg}}\\
		...
		\hline
		$\bullet$ vampire & \multirow{7}{*}{\includegraphics[width=.5in]{../kids/E1.jpg}}\\
		$\bullet$ hopping & \\
		...
		\hline
		$\bullet$ racecar & \multirow{7}{*}{\includegraphics[width=.5in]{../kids/D1.jpg}}\\
		$\bullet$ airplane &\\
		$\bullet$ alligator &\\
		...
		$\bullet$ person & \multirow{4}{*}{\includegraphics[width=.5in]{../kids/B1.jpg}}\\
...
```{r sim2, fig.cap = "Semantic similarity between descriptions from earlier blocks (1-3) and the last block in Experiment 2.  Heavy dots are means with bootstrapped 95% CIs; light dots are individual values. \\label{tolast}" }
sims_2 |>
  filter(game1 == game2) |>
